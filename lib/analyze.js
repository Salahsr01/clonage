...
async function analyzeDOM(page, outDir) {
  ...
  const analysis = await page.evaluate(({ MAX_DEPTH, MAX_SVG_BYTES }) => {
    ...
    /**
     * Extract AUTHORED CSS rules (the real selectors + authored values)
     * that match elements within a section. This gives the LLM
     * `width: 20vw` instead of `width: 384px`.
     */
    function collectAuthoredCSS(sectionEl) {
      const used = new Set();
    ...
    for (const [el, info] of sections) {
      ...
      const sectionData = {
        ...
        authoredCSS: collectAuthoredCSS(el),
  ...
  for (const section of analysis.sections) {
    ...
    // authored.css — REAL CSS rules (authored values, not computed)
    if (section.authoredCSS) {
      fs.writeFileSync(
        path.join(sectionDir, 'authored.css'),
        `/* Authored CSS rules matching ${section.role} */\n\n${section.authoredCSS}`,
        'utf-8'
      );
    }
